...
const ModuleLectures = () => {
  ...
  useEffect(() => {
    const fetchLectures = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_BACKEND_URI}/api/modules/${moduleId}/lectures`);
        ...
        if (response.data && response.data.success) {
          ...
          let lecturesData = response.data;
          let moduleInfoData = {};
          ...
          if (response.data && typeof response.data === 'object' && !Array.isArray(response.data)) {
            if (Array.isArray(response.data.lectures)) {
              ...
              lecturesData = response.data.items;
            } else {
        ...
        setLectures([
          {
            ...
            courseId: 'CS101',
            moduleId: 'MOD001',
            lectureName: 'Introduction to Variables',
            recordingId: 'REC001',
          ...
          },
          {
            ...
            courseId: 'CS101',
            moduleId: 'MOD001',
            lectureName: 'Control Structures',
            recordingId: 'REC002',
            recordingPath: 'https://www.w3schools.com/html/mov_bbb.mp4',
  ...
  // Additional security measures when lecture modal is open
  useEffect(() => {
    if (showLecture) {
  ...
  const closeLecture = () => {
    ...
    if (selectedLecture?.eventListeners) {
      document.removeEventListener('contextmenu', selectedLecture.eventListeners.preventDownload);
      document.removeEventListener('keydown', selectedLecture.eventListeners.preventDownload);
    }
            ...
            <div className="p-8">
              ...
              <div className="mb-8">
                ...
                <h1 className="text-3xl font-bold text-gray-900">
                  {moduleInfo?.moduleName ? `${moduleInfo.moduleName} Lecture Videos` : 'Module Lecture Videos'}
                </h1>
                {moduleInfo?.count && (
                  <p className="text-lg text-gray-600 mt-2">Total Lectures: {moduleInfo.count}</p>
                )}
              </div>
                ...
                <div className="space-y-6">
                  {lectures.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="mt-1 text-sm text-gray-500">No lecture videos have been uploaded for this module yet.</p>
                    </div>
                  ) : (
                            ...
                            <div className="flex-1">
                              <div className="mb-2">
                                <span className="text-sm font-medium text-crimson-600 bg-crimson-50 px-2 py-1 rounded">
                                  {lecture.recordingId || lecture.id}
                                </span>
                              </div>
              ...
              <div>
                ...
                <div className="flex items-center space-x-2 mt-1">
                  <p className="text-sm text-gray-500">
                    {selectedLecture.courseId || 'Unknown Course'}
                  </p>
